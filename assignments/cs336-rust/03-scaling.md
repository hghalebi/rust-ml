...
- **Objects:** `ExperimentConfig`, `TrainingRun`, `MetricRecord`, `ComputeBudgetFlops`, `ValidationLoss`, `ScalingFit`, `ScalingCandidate`, `ScalingTradeoff`, `MetricVisibility`, and `PublicScalingReport`.
- **Maps:** configure a run, record metrics, fit a curve, forecast loss, compare candidates, attach limitation notes, then review evidence for public release.
- **Composition path:** `ExperimentConfig -> TrainingRun -> MetricRecord -> ScalingFit -> ScalingCandidate -> ScalingTradeoff` and `ReviewedMetricRecord* -> PublicScalingReport`.
- **Invariant to protect with newtypes:** validation loss, parameter count, token count, compute budget, recommendation, and public-release class carry different evidence and cannot be swapped.
...
- one public report trace that rejects restricted or private metric records
...
The public-report arrow also preserves the rule that restricted or private
experiment evidence stays out of learner-facing material.
...
- reject restricted or private metric records at the public-report boundary
...
- **Public safety:** public reports are constructed only from reviewed public metric records.
...
- restricted or private experiment evidence can appear in learner-facing public reports
