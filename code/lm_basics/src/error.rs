//! Error types for the language-modeling basics teaching crate.
...
/// Shape observed while validating a language-modeling operation.
...
/// Missing token text observed at the vocabulary boundary.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct MissingTokenText(String);

impl MissingTokenText {
    pub(crate) fn new(value: String) -> Self {
        Self(value)
    }
}

impl fmt::Display for MissingTokenText {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        formatter.write_str(&self.0)
    }
}

/// Requested token id in a learner-facing vocabulary error.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord)]
pub struct RequestedTokenId(usize);

impl RequestedTokenId {
    pub(crate) fn new(value: usize) -> Self {
        Self(value)
    }
}

impl fmt::Display for RequestedTokenId {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        fmt::Display::fmt(&self.0, formatter)
    }
}

/// Vocabulary size attached to a token-id validation error.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord)]
pub struct VocabularySizeDiagnostic(usize);

impl VocabularySizeDiagnostic {
    pub(crate) fn new(value: usize) -> Self {
        Self(value)
    }
}

impl fmt::Display for VocabularySizeDiagnostic {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        fmt::Display::fmt(&self.0, formatter)
    }
}

/// Validation, shape, and numerical failures for tiny language modeling.
#[derive(Debug, Error, PartialEq)]
pub enum LmBasicsError {
    ...
    #[error("unknown token in {operation}: {token}")]
    UnknownToken {
        /// The operation that failed.
        operation: OperationName,
        /// The missing token text.
        token: MissingTokenText,
    },

    /// A token id was outside the vocabulary range.
    #[error("invalid token id in {operation}: id {id}, vocabulary size {vocab_size}")]
    InvalidTokenId {
        /// The operation that failed.
        operation: OperationName,
        /// Requested token id.
...
impl LmBasicsError {
    ...
    pub(crate) fn unknown_token(operation: &'static str, token: String) -> Self {
        Self::UnknownToken {
            operation: OperationName::new(operation),
            token: MissingTokenText::new(token),
        }
    }

    pub(crate) fn invalid_token_id(operation: &'static str, id: usize, vocab_size: usize) -> Self {
        Self::InvalidTokenId {
            operation: OperationName::new(operation),
