# neuron

Status: planned.

This directory will become the runnable crate for the first trainable model.

## Owns

- lesson module: [03 Neuron](../../lessons/03-neuron/README.md)
- related training concepts: [04 Learning](../../lessons/04-learning/README.md)

## Future Purpose

- implement a single neuron
- run a forward pass
- train on a tiny boolean dataset such as AND or OR

## Packaging Note

This stays as a documentation placeholder until the lesson has enough executable material to justify a real Cargo crate.
